...
<div style="max-width: 800px; margin: 0 auto;">
    ...
    <div style="background: #ffffff; border-radius: 12px; padding: 24px; border: 1px solid #cec4b0; box-shadow: 0 2px 8px rgba(0,0,0,0.06); margin-bottom: 20px;">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
            <div>
                <div style="font-size: 13px; color: #6e6559; margin-bottom: 4px;">订单号：{{ $order->order_no }}</div>
                <span style="display: inline-block; padding: 6px 18px; border-radius: 999px; font-size: 14px; font-weight: 600; letter-spacing: 0.03em; background: {{ $statusBg[$order->status] ?? '#f4f1ec' }}; color: {{ $statusColor[$order->status] ?? '#6e6559' }};">
    ...
    <div style="background: #ffffff; border-radius: 12px; border: 1px solid #cec4b0; box-shadow: 0 1px 2px rgba(0,0,0,0.04); margin-bottom: 20px; overflow: hidden;">
        <div style="padding: 16px 24px; font-size: 15px; font-weight: 600; color: #2c2416; border-bottom: 1px solid #cec4b0;">购买书籍</div>
        @foreach($order->items as $item)
            <div style="display: flex; gap: 16px; padding: 16px 24px; {{ !$loop->last ? 'border-bottom: 1px solid #cec4b0;' : '' }}">
    ...
    {{-- 订单信息 --}}
    <div style="background: #ffffff; border-radius: 12px; padding: 24px; border: 1px solid #cec4b0; box-shadow: 0 1px 2px rgba(0,0,0,0.04); margin-bottom: 20px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; font-size: 13px;">
            <div><span style="color: #6e6559;">取书地点：</span>{{ $order->pickup_location }}</div>
            <div><span style="color: #6e6559;">下单时间：</span>{{ $order->created_at->format('Y-m-d H:i') }}</div>
            @if($order->paid_at)
                <div><span style="color: #6e6559;">支付时间：</span>{{ $order->paid_at->format('Y-m-d H:i') }}</div>
            @endif
            @if($order->confirmed_at)
                <div><span style="color: #6e6559;">确认时间：</span>{{ $order->confirmed_at->format('Y-m-d H:i') }}</div>
            @endif
            @if($order->picked_up_at)
                <div><span style="color: #6e6559;">取书时间：</span>{{ $order->picked_up_at->format('Y-m-d H:i') }}</div>
            @endif
            @if($order->cancelled_at)
                <div><span style="color: #6e6559;">取消时间：</span>{{ $order->cancelled_at->format('Y-m-d H:i') }}</div>
                @if($order->cancel_reason)
                    <div><span style="color: #6e6559;">取消原因：</span>{{ $order->cancel_reason }}</div>
                @endif
            @endif
        </div>
    </div>
    ...
    @if($order->timeline->isNotEmpty())
        <div style="background: #ffffff; border-radius: 12px; padding: 24px; border: 1px solid #cec4b0; box-shadow: 0 1px 2px rgba(0,0,0,0.04);">
            ...
            <div style="position: relative; padding-left: 24px;">
                @foreach($order->timeline as $event)
                    <div style="position: relative; padding-bottom: {{ $loop->last ? '0' : '20px' }};">
                        <div style="position: absolute; left: -20px; top: 4px; width: 10px; height: 10px; border-radius: 50%; background: {{ $loop->first ? '#b49450' : '#8a8070' }}; border: 2px solid #ffffff; box-shadow: {{ $loop->first ? '0 0 0 3px rgba(180,148,80,0.25)' : '0 0 0 2px #cec4b0' }};"></div>
                        @if(!$loop->last)
                            <div style="position: absolute; left: -16px; top: 18px; width: 2px; height: calc(100% - 4px); background: #cec4b0;"></div>
                        @endif
                        <div style="font-size: 14px; color: #2c2416;">{{ $event->remark }}</div>
                        <div style="font-size: 12px; color: #6e6559; margin-top: 2px;">{{ $event->created_at->format('Y-m-d H:i') }}</div>
                    </div>
                @endforeach
            </div>
        </div>
