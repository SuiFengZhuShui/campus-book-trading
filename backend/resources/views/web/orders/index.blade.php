...
@section('content')
<a href="{{ request('from') === 'profile' ? '/profile' : '/' }}" style="display:inline-flex;align-items:center;gap:4px;padding:8px 18px;background:linear-gradient(135deg,#b49450,#d4bc7c);color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:500;text-decoration:none;margin-bottom:16px;">&larr; 返回</a>
<h2 style="font-size: 22px; font-weight: 600; color: #2c2416; letter-spacing: -0.01em; margin-bottom: 20px;">我的订单</h2>

@if($orders->count() > 0)
    <div style="display: flex; flex-direction: column; gap: 16px;">
        @foreach($orders as $order)
            <a href="/orders/{{ $order->id }}{{ request('from') === 'profile' ? '?from=profile' : '' }}" style="display: block; background: #ffffff; border-radius: 12px; padding: 20px 24px; border: 1px solid #cec4b0; box-shadow: 0 1px 2px rgba(0,0,0,0.04); border-left: 3px solid transparent; transition: box-shadow 0.25s, border-left-color 0.25s;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
                    ...
                    <div style="text-align: right;">
                        ...
                        @if($order->status === 'pending' && $order->buyer_id !== auth()->id())
                            <div style="font-size: 12px; color: #8c8478; margin-top: 4px;">等待买家付款</div>
                        @endif
                        <div style="font-size: 18px; font-weight: 400; color: #b49450; margin-top: 6px; font-family: 'LXGW WenKai', serif;">¥{{ $order->total_amount }}</div>
                    </div>
                </div>
            </a>
        @endforeach
